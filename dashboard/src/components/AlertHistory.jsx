...
export default function AlertHistory({ alerts }) {
  ...
  return (
    <div className="card p-5">
      <h2 className="text-zinc-700 dark:text-zinc-300 font-bold mb-4">Recent Alerts</h2>

      {recent.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-6 text-center">
          <span className="text-2xl">✓</span>
          <p className="text-zinc-400 text-sm font-medium">All caught up</p>
          <p className="text-zinc-300 dark:text-zinc-600 text-xs">No alerts fired yet.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-3 max-h-64 overflow-y-auto pr-1">
          {recent.map((a, i) => {
            ...
            return (
              <div key={i} className={`border-b border-zinc-100 dark:border-zinc-800 pb-3 last:border-0 last:pb-0 ${isStale ? 'opacity-40' : ''}`}>
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <span className="text-zinc-900 dark:text-zinc-100 text-sm font-semibold capitalize">{a.protocol}</span>
                ...
                <p className="text-zinc-500 dark:text-zinc-400 text-xs leading-relaxed">{a.message}</p>
              </div>
            )
          })}
        </div>
