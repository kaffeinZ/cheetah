...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen text-zinc-900 dark:text-zinc-100 flex flex-col">

      {/* Header */}
      <header className="bg-white dark:bg-[#1a1a24] border-b border-black/8 dark:border-white/7 px-6 py-4 flex items-center justify-between shadow-sm">
      ...
      {/* Status bar */}
      <div className="bg-white dark:bg-[#1a1a24] border-b border-black/8 dark:border-white/7 px-6 py-2 flex items-center justify-between text-xs text-zinc-400">
      ...
      {/* 2-column layout */}
      <main className="flex-1 w-full max-w-[1400px] mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-[3fr_1fr] gap-5 items-start bg-[#f8f8fc] dark:bg-[#0f0f13]">
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="card p-4 glow-cyan border-l-4 border-l-[#00c8e0]">
                  ...
                  <p className="text-zinc-900 dark:text-zinc-100 text-2xl font-bold">${portfolio.totalCollateralUsd?.toFixed(2) ?? '0.00'}</p>
                </div>
                <div className="card p-4 glow-pink border-l-4 border-l-[#e0007a]">
                  ...
                  <p className="text-zinc-900 dark:text-zinc-100 text-2xl font-bold">${portfolio.totalBorrowUsd?.toFixed(2) ?? '0.00'}</p>
                </div>
              </div>
              ...
              {portfolio.positions.length === 0 ? (
                <div className="flex flex-col items-center gap-4 py-16 text-center">
                  <p className="text-zinc-400">No active positions found.</p>
                  <p className="text-zinc-300 text-sm">Make sure your wallet has open positions on MarginFi, Kamino, or Jupiter.</p>
                </div>
              ) : (
                <div className="flex flex-col gap-4">
                  <h2 className="text-zinc-400 font-semibold uppercase tracking-wider text-xs">Active Positions</h2>
                  {/* Perps first, then lending */}
                  {[...portfolio.positions].sort((a, b) => (a.positionType === 'perp' ? -1 : 1)).map((p, i) => (
                    p.positionType === 'perp'
        ...
        {/* Right — Alerts + Calc + History + Settings */}
